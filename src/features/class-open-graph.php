...
final class Open_Graph implements Feature {
	...
	protected static function get_default_image( $post_id ): string|bool {
		...
		$default_image_url = ( ! empty( $default_image_id ) && is_numeric( $default_image_id ) )
			? wp_get_attachment_image_url( (int) $default_image_id, 'full' )
			: false;
	...
	public function render_open_graph_tags(): void {
		...
		if ( is_front_page() && ! ( is_singular() && post_type_supports( (string) get_post_type(), 'wp-seo-open-graph' ) ) ) {
