...
Decision: build a bespoke page instead, reading from the feature Registry ([ADR 0002](0002-bespoke-feature-wrapper-for-gating-and-registry.md)), and give it a new top-level "WP SEO" admin menu rather than nesting it under Core's Settings sidebar. This is more work than a Site Health entry, and is expected to be superseded by the v2.1 toggle UI, but a dedicated, discoverable location beats a debug screen most site admins never open, and a top-level menu leaves room for the toggle UI and anything else the plugin surfaces later.
